  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      ...
      <div className="flex flex-col gap-4 w-full sm:max-w-[480px]">
         ...
         <input required onChange={onchangeHandler} name='email' value={formData.email} className="border border-gray-400 rounded py-1.5 px-3.5 w-full" type="text" placeholder="Email address"/>
         <input required onChange={onchangeHandler} name='street' value={formData.street} className="border border-gray-400 rounded py-1.5 px-3.5 w-full" type="text" placeholder="Street"/>
            ...
            <div className="flex gap-3">
              ...
              <input required onChange={onchangeHandler} name='country' value={formData.country} className='border border-gray-400 rounded py-1.5 px-3.5 w-full' type="text" placeholder='Last name' />
           </div>
      ...
      <div className="mt-8">
        ...
        <div className="mt-12">
          ...
          <div onClick={()=>setMethod('stripe')} className="flex items-center gap-3 boder p-2 px-3 cursor-pointer">
               <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'stripe' ? 'bg-green-500': ''}`}></p>
               <img className='h-5 mx-4 ' src={assets.stripe_logo} alt="" />
             </div>
             {/* <div onClick={()=>setMethod('razorpay')} className="flex items-center gap-3 boder p-2 px-3 cursor-pointer">
               <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'razorpay' ? 'bg-green-500': ''}`}></p>
               <img className='h-5 mx-4 ' src={assets.razorpay_logo} alt="" />
             </div> */}
             <div onClick={()=>setMethod('cod')} className="flex items-center gap-3 boder p-2 px-3 cursor-pointer">
               <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'cod' ? 'bg-green-500': ''}`}></p>
               <p className='text-gray-500 text-sm font-medium mx-4'>CASH ON DELIVERY</p>
             </div>
          </div>
<div className="w-full text-end mt-8">
  <button type='submit'  className='mt-0 border border-black text-white bg-black text-sm hover:bg-lime-500 hover:text-black transition-all duration-500 rounded-lg px-10 py-3 '>PLACE ORDER</button>
</div>
